...
<xsl:output method="html" doctype-system="http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"/>
	<xsl:template match="/">  
		...
		<xsl:if test="rss/channel/item/category = $category" >
			<h3>Volunteer Leaders</h3>
			<!--For each post must include whether Volunteer Leader Image or Volunteer Leader Video-->
			<xsl:for-each select="rss/channel/item[category='Volunteer Leader Image' and category=$category]" >
			...
			<xsl:for-each select="rss/channel/item[category='Volunteer Leader Video' and category=$category]" >
			<div style="background-color:#FFFFCC;width:100%;height:240px;margin-bottom:10px;">
